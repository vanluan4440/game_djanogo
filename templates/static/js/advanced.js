        ...
        _createClass(SnakeGame, [{
            ...
            },
            {
                ...
                value: function addTileToCanvas(tpx, tpy, sx, sy) {
                    ...
                    return this.ctx.drawImage(this.images[SNAKE_SPRITE], tpx * 64, tpy * 64, 64, 64, sx, sy, this.tileSize + 2, this.tileSize + 2);
                }
            ...
            {
                key: "addTileToApple",
                value: function addTileToApple(tpx, tpy, sx, sy) {
                    // image, posX in tile.png, posY in tile.png, tileWidth, tileHeight, posX in canvas, posY in canvas, tileSize in canvas = 25
                    return this.ctx.drawImage(this.images[SNAKE_SPRITE], tpx * 64, tpy * 64, 64, 64, sx, sy, this.tileSize + 5, this.tileSize + 10);
...
function getRoundAndId() {
    ...
    $.ajax({
        type: 'POST',
        url: `${window.location.origin}/api/getRoundAndId`,
        data: {
            round: url.get('round'),
            level: url.get('level'),
        },
...
function UpdateMarkByRoundIDAndLevel(point) {
    ...
    const url = new URLSearchParams(queryString)
    $.ajax({
        url: `${window.location.origin}/api/UpdateMarkByRoundIDAndLevel`,
        type: 'POST',
        data: {
            round: url.get('round'),
            level: url.get('level'),
